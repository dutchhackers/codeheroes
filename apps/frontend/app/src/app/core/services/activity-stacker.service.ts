...
import { PR_STACKABLE_ACTION_TYPES, PRFinalState } from '../constants/stack.constants';
import { ActivityStack, FeedItem, SingleActivity } from '../models/activity-stack.model';
...
export class ActivityStackerService {
  ...
  private createStack(stackKey: string, activities: Activity[]): ActivityStack {
    ...
    // Calculate total XP
    const totalXp = gameActivities.reduce((sum, activity) => sum + (activity.xp?.earned ?? 0), 0);
