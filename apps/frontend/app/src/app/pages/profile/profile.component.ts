...
@Component({
  ...
  template: `
      ...
      <div class="flex items-center justify-between relative z-10">
        ...
        <div class="flex items-center gap-2">
          <button
              ...
              aria-label="Settings"
              class="action-button rounded bg-black/50 border border-white/20 text-slate-400 hover:text-white hover:border-white/40 transition-colors"
            >
    ...
    @if (showSettingsModal()) {
      <app-settings-modal
        [userId]="user()?.id ?? ''"
        [initialNotificationsEnabled]="user()?.preferences?.notificationsEnabled ?? true"
        (dismiss)="closeSettingsModal()"
      />
    }
  `,
  styles: [
