...
export function printHeader(): void {
  console.log(`\n${COLORS.bright}GitHub Event Simulator${COLORS.reset}`);
  console.log('━'.repeat(40));
}

export function printEventInfo(info: EventInfo): void {
  console.log(`\n${COLORS.cyan}Event:${COLORS.reset}       ${info.eventType}${info.action ? ` (${info.action})` : ''}`);
...
export function printSending(url: string, deliveryId: string): void {
  console.log(`\n${COLORS.dim}Delivery ID: ${deliveryId}${COLORS.reset}`);
