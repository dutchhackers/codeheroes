...
import { buildPullRequestOpenedPayload, PullRequestOptions } from '../payloads/pull-request';
...
export function createPrCreatedCommand(
  ...
  const command = new Command('pr-created')
    .description('Simulate a Bitbucket Server pull request opened event')
    ...
    .action(async (cmdOptions) => {
      ...
      try {
        ...
        const payload = buildPullRequestOpenedPayload(config, prOptions);

        const info: EventInfo = {
          eventType: 'pr:opened',
          details: {
            title: payload.pullRequest.title,
            source: payload.pullRequest.fromRef.displayId,
            target: payload.pullRequest.toRef.displayId,
        ...
        const result = await sendWebhook('pr:opened', payload);
