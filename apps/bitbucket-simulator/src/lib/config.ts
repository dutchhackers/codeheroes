...
export interface BitbucketUserConfig {
  accountId: string;
  displayName: string;
  nickname: string;
  uuid: string;
...
function validateConfig(config: Config): void {
  if (!config.bitbucket) {
    throw new Error('Config missing "bitbucket" section');
  }
  if (!config.bitbucket.accountId || !config.bitbucket.nickname) {
    throw new Error('Config missing required bitbucket fields: accountId, nickname');
  }
