import { BitbucketCloudAdapter } from '../adapter';
import { BitbucketCloudPushWebhook, BitbucketCloudPullRequestWebhook } from '../types';

describe('BitbucketCloudAdapter', () => {
  let adapter: BitbucketCloudAdapter;

  beforeEach(() => {
    adapter = new BitbucketCloudAdapter();
  });

  describe('validateWebhook', () => {
    it('should validate a valid Bitbucket Cloud webhook', () => {
  ...
  describe('mapEventToGameAction - push events', () => {
    const basePushWebhook: BitbucketCloudPushWebhook = {
      actor: {
    ...
    it('should map push event to code_push action', () => {
      ...
      expect((result as any)?.provider).toBe('bitbucket_cloud');
  ...
  describe('mapEventToGameAction - pull request events', () => {
    const basePrWebhook: BitbucketCloudPullRequestWebhook = {
      actor: {
        account_id: '557058:abc-123',
        display_name: 'Test User',
        nickname: 'testuser',
    ...
    it('should map PR fulfilled event to pull_request_merge', () => {
      const mergedPr: BitbucketCloudPullRequestWebhook = {
        ...basePrWebhook,
        pullrequest: {
