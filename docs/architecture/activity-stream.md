...
**Important:** For `game-action` activities, the Firestore document ID uses the pattern generated by `ActivityRecorderService` (e.g., `act_{timestamp}_{actionId}`). Activities are persisted within a transaction in `ProgressionRepository.updateState()`, using the pre-generated ID as the document ID. This ensures activity and progression state are written atomically and eliminates duplicates that could be introduced by the previous non-transactional double-write path, but it does not by itself make game-action processing fully idempotent.

The table below shows the ID patterns used in the codebase:

| Type | Pattern | Example | Firestore ID |
|------|---------|---------|--------------|
| Game Action | `act_{timestamp}_{actionId}` | `act_1706745600000_ga_12345` | Same as pattern |
| Manual Activity | `manual_{timestamp}_{userId}` | `manual_1706745600000_user123` | As generated |
| Badge Earned | `badge_{userId}_{badgeId}[_level{level} \| _count{count}]` | `badge_user123_first_push_level2` | Same as pattern (generated by `RewardActivityService`) |
| Level Up | `levelup_{userId}_{newLevel}` | `levelup_user123_10` | Same as pattern (generated by `RewardActivityService`) |
...
3. **ProgressionService** calculates XP and creates activity record with pre-generated ID
4. **ActivityRecorderService.createFromAction()** generates the activity record with ID pattern `act_{timestamp}_{actionId}`
5. **ProgressionRepository.updateState()** writes the activity atomically within a Firestore transaction to `users/{userId}/activities/{activityId}` along with progression state updates
