...
**Important:** For `game-action` activities, the Firestore document ID uses the pattern generated by `ActivityRecorderService` (e.g., `act_{timestamp}_{actionId}`). Activities are persisted within a transaction in `ProgressionRepository.updateState()`, using the pre-generated ID as the document ID. This ensures atomic writes and prevents duplicate records.
